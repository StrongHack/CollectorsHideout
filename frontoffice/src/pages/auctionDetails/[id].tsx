...
import { AuctionType, BidStatus } from "../../../types";
import { formatDate } from "../../../constants/global";
import BidLogic from "../../../components/bidLogic";
...
export default function Home() {
  ...
  /**
   * Updates the bid data (minBid, HighestBid and button state) in real time for the auction
   *
   * @param auctionId
   */
  const updateBidData = async (auctionId: string) => {
    try {
      const updatedAuction = await AuctionsService.getAuction(auctionId);
      ...
      setAuction((prevAuctions) => {
        ...
        return prevAuctions.map((auction) => {
          if (auction[".id"] === auctionId) {
            if (
              auction.auctionHighestBid !== updatedAuction.auctionHighestBid ||
              auction.auctionMinimumBid !== updatedAuction.auctionMinimumBid
            ) {
              return {
                ...auction,
                auctionHighestBid: updatedAuction.auctionHighestBid,
                auctionMinimumBid: updatedAuction.auctionMinimumBid,
              };
            }
          }
          return auction;
      ...
      const endDate = new Date(updatedAuction.auctionEndDate);
      const currentDate = new Date();

      if (endDate <= currentDate) {
        await AuctionsService.updateAuctionState(auctionId);
      }
    } catch (error) {
        ...
        if (router.query.id) {
          AuctionsService.getAuction(router.query.id as string).then(
            (auctionData) => {
              ...
              updateBidData(router.query.id as string);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-5">
          ...
          <main>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex-1 flex flex-wrap gap-2 justify-start mb-5">
                <BidLogic
                  updateBid={() => updateBidData(auction![0][".id"])}
                  auction={auction?.[0] ?? null}
                  bids={auction?.[0]?.bids ?? undefined}
                />
              </div>
            </div>
          </main>
